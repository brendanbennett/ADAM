...
def Read_Shape(shapefile):
    ...
    for i in range(0, nfac):
        ...
        svert = s.split()
        tlist[i,] = np.asarray(
            [int(svert[0]), int(svert[1]), int(svert[2])], dtype=np.int
        )
    f.close()
    return tlist, vlist
    ...
    for n in range(0, nOCC):
        ...
        for j in range(0, nchords):
            ...
            [
                ...
                [
                    float(svert[0]) - MinTim,
                    float(svert[1]),
                    float(svert[2]),
                    float(svert[3]),
                    float(svert[4]) - MinTim,
                    float(svert[5]),
                    float(svert[6]),
                    float(svert[7]),
                ]
...
def plot_occ(tlist, vlist, angles, offset, OCCs, index):
    ...
    plt.triplot(
        np.array(vlist2[:, 0]).flatten(),
        np.array(vlist2[:, 1]).flatten(),
        tlist - 1,
        "g-",
        alpha=0.5,
    )
...
def Local_Horizon(tlist, vlist, facindex, v2fmatrix):
    ...
    nfac = tlist.shape[0]
    normal = np.cross(
        vlist[tlist[facindex, 1], :] - vlist[tlist[facindex, 0], :],
        vlist[tlist[facindex, 2], :] - vlist[tlist[facindex, 0], :],
    )
    cent = (
        vlist[tlist[facindex, 0], :]
        + vlist[tlist[facindex, 1], :]
        + vlist[tlist[facindex, 2], :]
    ) / 3
...
def is_in_triangle(point, dir, v1, v2, v3):
    ...
    if abs(det) < 1e-6:
        return 0
    gamma = (
        dir[2] * (s1[0] * b[1] - b[0] * s1[1])
        + dir[1] * (b[0] * s1[2] - s1[0] * b[2])
        + dir[0] * (s1[1] * b[2] - b[1] * s1[2])
    ) / det
    if gamma < 0 or gamma > 1:
        return 0
    beta = (
        b[0] * (s2[1] * dir[2] - dir[1] * s2[2])
        + b[1] * (dir[0] * s2[2] - s2[0] * dir[2])
        + b[2] * (s2[0] * dir[1] - s2[1] * dir[0])
    ) / det
